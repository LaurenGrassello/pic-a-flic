...
private function mixedFeed(Request $req, Response $res, string $order = 'popularity'): Response
{
    ...
    if (!in_array($type, ['all','movie','tv'], true)) $type = 'all';
    $wantMovies = $type !== 'tv';
    $wantTv     = $type !== 'movie';
    ...
    $lowerCols = function (string $table) use ($schema): array {
        try { return array_map(fn($c)=>strtolower($c->getName()), $schema->listTableColumns($table)); }
...
public function search(Request $req, Response $res): Response {
    ...
    try {
        if ($hasM && $wantM) {
            $qm = $conn->createQueryBuilder()
                ->select("
                    m.`" . (function() use ($schema){ $c=array_map(fn($c)=>strtolower($c->getName()),$schema->listTableColumns('movies')); return in_array('tmdb_id',$c,true)?'tmdb_id':'id'; })() . "` AS tmdb_id,
                    0 AS is_tv,
                    ...
                    " . (function() use ($schema){
                        ...
                        foreach (['popularity','vote_average','vote_count'] as $k) if (in_array($k,$c,true)) return "m.`$k`";
                        return "1";
                    })() . " AS popularity,
                    NULL AS release_date,
                    " . (function() use ($schema){
                        $c=array_map(fn($c)=>strtolower($c->getName()),$schema->listTableColumns('movies'));
                        foreach (['poster_path','poster','poster_url','posterurl'] as $k) if (in_array($k,$c,true)) return "m.`$k`";
                        return "NULL";
                    })() . " AS poster_path
                ")
                ->from('movies','m')
        ...
        if ($hasT && $wantT) {
            $qt = $conn->createQueryBuilder()
                ->select("
                    t.`" . (function() use ($schema){ $c=array_map(fn($c)=>strtolower($c->getName()),$schema->listTableColumns('tv_shows')); return in_array('tmdb_id',$c,true)?'tmdb_id':'id'; })() . "` AS tmdb_id,
                    1 AS is_tv,
                    ...
                    " . (function() use ($schema){
                        ...
                        foreach (['popularity','vote_average','vote_count'] as $k) if (in_array($k,$c,true)) return "t.`$k`";
                        return "1";
                    })() . " AS popularity,
                    NULL AS release_date,
                    " . (function() use ($schema){
                        $c=array_map(fn($c)=>strtolower($c->getName()),$schema->listTableColumns('tv_shows'));
                        foreach (['poster_path','poster','poster_url','posterurl'] as $k) if (in_array($k,$c,true)) return "t.`$k`";
                        return "NULL";
                    })() . " AS poster_path
                ")
                ->from('tv_shows','t')
    ...
    $out = [];
    if ($wantM) $out = array_merge($out, $take('movie'));
    if ($wantT) $out = array_merge($out, $take('tv'));
...
public function titleProviders(Request $req, Response $res, array $args): Response {
    ...
    $id   = (int)($args['id'] ?? 0);
    if ($id <= 0) return $this->json($res, ['error' => 'Bad id'], 400);
...
private function discoverFeed(Request $req, Response $res, string $order='popularity'): Response {
    ...
    $collect = function (string $type) use ($provIds, $region, $pageUi, $limit) {
        ...
        while (count($rows) < $limit) {
            ...
            if ($provStr) $qs['with_watch_providers'] = $provStr;
